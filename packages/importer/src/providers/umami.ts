...
import { getReferrerWithQuery, parseReferrer } from '../utils/referrer-parser';
...
export class UmamiProvider extends BaseImportProvider<UmamiRawEvent> {
  ...
  async *parseSource(): AsyncGenerator<UmamiRawEvent, void, unknown> {
    const isRemote = this.isRemoteFile(this.config.fileUrl);

    if (isRemote) {
      yield* this.parseRemoteFile(this.config.fileUrl);
    } else {
      yield* this.parseLocalFile(this.config.fileUrl);
    }
  }

  private isRemoteFile(sourceLocation: string): boolean {
    return (
      sourceLocation.startsWith('http://') ||
      sourceLocation.startsWith('https://')
    );
  }
  ...
  private async *parseLocalFile(
    ...
    // Build pipeline with optional decompression
    (async () => {
      try {
        if (isGzipped) {
          await pipeline(fileStream, createGunzip(), parser);
        } else {
    ...
    try {
      ...
      throw new Error(
        `Failed to parse local file ${filePath}: ${
  ...
  transformEvent(_rawEvent: UmamiRawEvent): IClickhouseEvent {
    ...
    return {
      ...
      session_id: rawEvent.session_id || '', // Use Umami's session_id directly
      properties,
