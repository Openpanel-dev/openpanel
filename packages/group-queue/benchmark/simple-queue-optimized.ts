...
export async function benchmarkSimpleQueueOptimized() {
  console.log('🚀 Starting Optimized Simple Queue Benchmark...');

  const redis = new Redis(REDIS_URL, {
    maxRetriesPerRequest: null,
  });
  const namespace = 'benchmark:simple-opt:' + Date.now();
  ...
  for (let i = 0; i < WORKER_COUNT; i++) {
    const worker = new Worker<{ id: number }>({
      ...
      useBlocking: false, // Disable blocking for pure polling comparison
  ...
  console.log('✅ Optimized Simple Queue Results:');
