 ...
 *
 * events_tmp3 holds the Databricks backfill: one `_first_event` per device
 * (canonical profile_id = backend_userid -> userid -> first profile, the stable
 * deviceUID in both device_id and properties['deviceUID'], plus the first
 * session_id), Feb–Jun 2026, ~13.7M rows.
 *
 * We copy it back into `events` per-date so each INSERT stays small/safe on the
 * production table, dedup with `LIMIT 1 BY` as a guard, and skip dates that are
 * already present so re-runs are idempotent.
 *
 * Inspired by migration 14 (per-date move) and 16 (LIMIT 1 BY dedup).
 *
 * Usage:
 *   Dry run (one date):
 *     pnpm migrate:deploy:code -- 18 --cluster --dry --date=2026-02-01 --no-record
 *
 *   Execute one date:
 *     pnpm migrate:deploy:code -- 18 --cluster --date=2026-02-01 --no-record
 *
 *   All dates (shell loop, Feb 1 -> Jun 5):
 *     d=2026-02-01; while [ "$d" != 2026-06-06 ]; do \
 *       pnpm migrate:deploy:code -- 18 --cluster --date=$d --no-record; \
...
export async function up() {
  ...
  // This is a MANUAL, per-date migration (run via the move-first-event Job with
  // --date). When the automatic `migrate:deploy` loop imports it WITHOUT --date,
  // no-op gracefully so it can never crash the deploy migration job.
  if (!date) {
    console.log(
      '[18-move-first-event] No --date provided — manual per-date migration, skipping.',
    );
    console.log(
      '   Run via: pnpm migrate:deploy:code -- 18 --cluster --date=2026-02-01 --no-record',
    );
    return;
  }
  ...
  // Step 0: source counts in events_tmp3 for this date
  ...
  const srcResult = await chMigrationClient.query({
    query: `
      ...
      FROM ${SRC_TABLE}
      WHERE name = '${EVENT_NAME}' AND toDate(created_at) = '${date}'`,
    format: 'JSONEachRow',
  });
  ...
  // Step 1: idempotency guard - already present in events?
  ...
  if (existing > 0) {
    console.log(
      `\n  WARNING: ${DST_TABLE} already has ${existing.toLocaleString()} ${EVENT_NAME} for ${date}.`,
    );
    console.log(
  ...
  console.log(
    dstTotal === srcUnique ? '  MATCH' : '  MISMATCH - investigate before continuing',
  );
