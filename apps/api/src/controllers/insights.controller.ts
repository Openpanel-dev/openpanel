...
function getProjectId(req: RequestWithProjectParam): Promise<string> {
  const client = req.client!;
  return resolveClientProjectId({
    clientType: client.type === 'root' ? 'root' : 'read',
    clientProjectId: client.projectId ?? null,
    organizationId: client.organizationId,
    inputProjectId: req.params.projectId,
  });
}
...
export async function getOverview(
  ...
  return reply.send(
    await getAnalyticsOverviewCore({ projectId, startDate, endDate, interval: req.query.interval })
  );
}
...
export async function getMetrics(
  ...
  const projectId = await getProjectId(req as RequestWithProjectParam);
  const { timezone } = await getSettingsForProject(projectId);
  const { startDate, endDate } = getChartStartEndDate(req.query, timezone);
  reply.send(
...
export async function getPages(
  ...
  const { timezone } = await getSettingsForProject(projectId);
  const { startDate, endDate } = getChartStartEndDate(req.query, timezone);
  return overviewService.getTopPages({ projectId, filters: req.query.filters, startDate, endDate, timezone });
}
...
export async function getPagePerformance(
  ...
  const projectId = await getProjectId(req as RequestWithProjectParam);
  const { startDate, endDate } = await resolveDates(projectId, req.query);
  return reply.send(await getPagePerformanceCore({ projectId, startDate, endDate, ...req.query }));
}

// ---------------------------------------------------------------------------
// Analytics — overview generic (legacy)
// ---------------------------------------------------------------------------
...
export async function getFunnel(
  ...
  const { startDate, endDate } = await resolveDates(projectId, req.query);
  return reply.send(
    await getFunnelCore({ projectId, startDate, endDate, steps: req.query.steps, windowHours: req.query.windowHours, groupBy: req.query.groupBy })
  );
}
...
export const zReferrerQuery = zDateRange.extend({ breakdown: z.enum(referrerColumns).default('referrer_name') });
export const zGeoQuery = zDateRange.extend({ breakdown: z.enum(geoColumns).default('country') });
export const zDeviceQuery = zDateRange.extend({ breakdown: z.enum(deviceColumns).default('device') });
...
export async function getTrafficReferrers(
  ...
  const projectId = await getProjectId(req as RequestWithProjectParam);
  const { startDate, endDate } = await resolveDates(projectId, req.query);
  return reply.send(await getTrafficBreakdownCore({ projectId, startDate, endDate, column: req.query.breakdown as TrafficColumn }));
}
...
export const zUserFlowQuery = zDateRange.extend({
  ...
  exclude: z.union([z.array(z.string()), z.string().transform((s) => [s])]).optional(),
  include: z.union([z.array(z.string()), z.string().transform((s) => [s])]).optional(),
});
...
export async function getUserFlow(
  ...
  const projectId = await getProjectId(req as RequestWithProjectParam);
  const { startDate, endDate } = await resolveDates(projectId, req.query);
  return reply.send(await getUserFlowCore({ projectId, startDate, endDate, ...req.query }));
}
...
export const zEventsQuery = zDateRange.extend({
  eventNames: z.union([z.array(z.string()), z.string().transform((s) => [s])]).optional(),
...
export const zPropertyValuesQuery = z.object({ eventName: z.string(), propertyKey: z.string() });
...
export const zGetProfileQuery = z.object({ eventLimit: z.number().int().min(1).max(100).default(20) });
...
export const zProfileSessionsQuery = z.object({ limit: z.number().int().min(1).max(100).default(20) });
...
export async function getProfileMetrics(
  ...
  const projectId = await getProjectId(req as RequestWithProjectParam);
  return reply.send(await getProfileMetricsCore({ projectId, profileId: req.params.profileId }));
}
...
export const zGetGroupQuery = z.object({ memberLimit: z.number().int().min(1).max(50).default(10) });
...
export async function getGroup(
  ...
  const projectId = await getProjectId(req as RequestWithProjectParam);
  return reply.send(await getGroupCore({ projectId, groupId: req.params.groupId, memberLimit: req.query.memberLimit }));
}
...
export async function listDashboards(
  ...
  const projectId = await getProjectId(req as RequestWithProjectParam);
  return reply.send(await listDashboardsCore({ projectId, organizationId: getOrgId(req as RequestWithProjectParam) }));
}
...
export async function listReports(
  ...
  const projectId = await getProjectId(req as RequestWithProjectParam);
  return reply.send(await listReportsCore({ projectId, dashboardId: req.params.dashboardId, organizationId: getOrgId(req as RequestWithProjectParam) }));
}
...
export async function getReportData(
  ...
  const projectId = await getProjectId(req as RequestWithProjectParam);
  return reply.send(await getReportDataCore({ projectId, reportId: req.params.reportId, organizationId: getOrgId(req as RequestWithProjectParam) }));
}
