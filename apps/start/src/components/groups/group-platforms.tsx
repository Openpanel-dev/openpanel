...
import { HelpCircle, Monitor, Server, Smartphone } from 'lucide-react';
...
/**
 * "Platforms" card for a group — aggregates every member's sessions
 * by SDK so you can see the web-vs-app split for this team at a
 * glance. Mirrors the profile version visually so the same mental
 * model works at both levels.
 */
export function GroupPlatforms({ groupId, projectId }: Props) {
  const trpc = useTRPC();
  ...
  return (
    <Widget className="w-full">
      <WidgetHead>
        <div className="title">Platforms (team-wide)</div>
      </WidgetHead>
