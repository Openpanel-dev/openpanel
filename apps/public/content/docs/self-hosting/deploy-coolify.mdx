...
- **Environment Variables**: Configure optional settings like:
  ...
  - `OPENAI_API_KEY` and/or `ANTHROPIC_API_KEY`: API keys for the in-app AI chat assistant (optional — set either or both)
...
- `ENCRYPTION_KEY`: Automatically generated key (shared by the API and worker)
...
The compose template exposes the following inputs for [social login](/docs/self-hosting/social-login) and [Google Search Console](/docs/self-hosting/google-search-console); Coolify maps each one to the containers that need it:

- `OPENPANEL_GOOGLE_CLIENT_ID`, `OPENPANEL_GOOGLE_CLIENT_SECRET`, `OPENPANEL_GOOGLE_REDIRECT_URI`
- `OPENPANEL_GITHUB_CLIENT_ID`, `OPENPANEL_GITHUB_CLIENT_SECRET`, `OPENPANEL_GITHUB_REDIRECT_URI`
- `OPENPANEL_GSC_GOOGLE_REDIRECT_URI`

The redirect URIs use the API's FQDN, e.g. `https://your-domain.com/api/oauth/google/callback`.
...
- [Enable social login](/docs/self-hosting/social-login) with Google or GitHub
- [Connect Google Search Console](/docs/self-hosting/google-search-console) for SEO data
