...
export async function generateMetadata({
  ...
  if (!data) {
    ...
    return {
      title: t('not_found_comparison_title'),
    };
  }
...
export default async function ComparePage({
  ...
  const pageUrl = url(localizedHref(`/compare/${slug}`, locale));
  ...
  return (
    <div>
      ...
      {data.migration && (
        <div id="migration">
          <MigrationSection locale={locale} migration={data.migration} />
        </div>
      )}
      ...
      {data.related_links && (
        <RelatedLinksSection locale={locale} relatedLinks={data.related_links} />
      )}
      ...
      <CtaBanner
        ...
        title={t('compare_switch_cta_title')}
      />
    </div>
  );
}
