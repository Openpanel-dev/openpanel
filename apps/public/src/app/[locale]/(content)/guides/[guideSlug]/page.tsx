...
import { getGuidePage, getGuidePages } from '@/lib/source';
...
export async function generateMetadata({
  ...
  const guide = getGuidePage([guideSlug], locale);

  if (!guide) {
    const t = await getTranslations('pages');
    return {
      title: t('not_found_guide_title'),
    };
  }
...
export default async function Page({
  ...
  const goBackUrl = localizedHref('/guides', locale);

  const relatedGuides = getGuidePages(locale)
    .filter(
      (item) =>
  ...
  const jsonLd = {
    ...
    step: guide?.data.steps.map((step, i) => ({
      ...
      position: i + 1,
      name: step.name,
      url: url(`${localizedHref(`/guides/${slug}`, locale)}#${step.anchor}`),
    })),
  };
