...
export default function SiteCheckerPage() {
  ...
  const staticCopy = {
    title: t('tools_url_checker_title'),
    description: t('tools_url_checker_description_long'),
    checking: t('tools_url_checker_checking'),
    check: t('tools_url_checker_check'),
    bodyTitle: t('tools_url_checker_body_title'),
    bodyDescription: t('tools_url_checker_body_description'),
    sections: Array.from({ length: 6 }).map((_, index) => {
      const n = index + 1;
      return [
        t(`tools_url_checker_section_${n}_title`),
        t(`tools_url_checker_section_${n}_description`),
      ] as const;
    }),
    faqTitle: t('tools_url_checker_faq_title'),
    faq: Array.from({ length: 7 }).map((_, index) => {
      const n = index + 1;
      return [
        t(`tools_url_checker_faq_${n}_question`),
        t(`tools_url_checker_faq_${n}_answer`),
      ] as const;
    }),
    finalTitle: t('tools_url_checker_final_title'),
    finalDescription: t('tools_url_checker_final_description'),
  };
  ...
  return (
    <div className="max-w-4xl">
      <SectionHeader
        title={staticCopy?.title ?? 'URL Checker'}
        description={
          ...
          <Button type="submit" disabled={loading} size="lg">
            {loading ? (
              <>
                ...
                {staticCopy?.checking ?? 'Checking...'}
              </>
            ) : (
              <>
                <Search className="size-4" />
                {staticCopy?.check ?? 'Check'}
              </>
            )}
          </Button>
        </div>
