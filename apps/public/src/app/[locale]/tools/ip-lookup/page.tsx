...
export default function IPLookupPage() {
  ...
  const staticCopy = {
    title: t('tools_ip_lookup_title'),
    description: t('tools_ip_lookup_description_long'),
    placeholder: t('tools_ip_lookup_placeholder'),
    detecting: t('tools_ip_lookup_detecting'),
    lookingUp: t('tools_ip_lookup_looking_up'),
    lookup: t('tools_ip_lookup_lookup'),
    bodyTitle: t('tools_ip_lookup_body_title'),
    bodyDescription: t('tools_ip_lookup_body_description'),
    sections: Array.from({ length: 5 }).map((_, index) => {
      const n = index + 1;
      return [
        t(`tools_ip_lookup_section_${n}_title`),
        t(`tools_ip_lookup_section_${n}_description`),
      ] as const;
    }),
    faqTitle: t('tools_ip_lookup_faq_title'),
    faq: Array.from({ length: 5 }).map((_, index) => {
      const n = index + 1;
      return [
        t(`tools_ip_lookup_faq_${n}_question`),
        t(`tools_ip_lookup_faq_${n}_answer`),
      ] as const;
    }),
    finalTitle: t('tools_ip_lookup_final_title'),
    finalDescription: t('tools_ip_lookup_final_description'),
  };
  ...
  return (
    <div className="max-w-4xl">
      <SectionHeader
        title={staticCopy?.title ?? 'IP Lookup Tool'}
        description={
      ...
      <form onSubmit={handleSubmit} className="mt-8">
        <div className="flex gap-2">
          <Input
            type="text"
            placeholder={
              staticCopy?.placeholder ??
              'Enter IP address or leave empty to detect yours'
            }
            value={ip}
          ...
          <Button type="submit" disabled={loading || autoDetecting} size="lg">
            {loading || autoDetecting ? (
              <>
                ...
                {autoDetecting
                  ? (staticCopy?.detecting ?? 'Detecting...')
                  : (staticCopy?.lookingUp ?? 'Looking up...')}
              </>
            ) : (
              <>
                <Search className="size-4" />
                {staticCopy?.lookup ?? 'Lookup'}
              </>
            )}
          </Button>
        </div>
          ...
          <article className="space-y-8">
            <div>
              <h2 className="text-3xl font-bold mb-4">
                {staticCopy.bodyTitle}
              </h2>
              <p className="text-lg text-muted-foreground mb-6">
                {staticCopy.bodyDescription}
              </p>
            </div>
            {staticCopy.sections.map(([title, body]) => (
              <section key={title}>
            ...
            <section>
              ...
              <Faqs>
                {staticCopy.faq.map(([question, answer]) => (
            ...
            <section className="border-t pt-8 mt-8">
              <h3 className="text-2xl font-semibold mb-4">
                {staticCopy.finalTitle}
              </h3>
              <p className="mb-6">{staticCopy.finalDescription}</p>
            </section>
          </article>
        ) : (
          <article className="space-y-8">
