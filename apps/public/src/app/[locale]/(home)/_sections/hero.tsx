...
const perks = [
  { key: 'trial', icon: CalendarIcon },
  { key: 'no_card', icon: CreditCardIcon },
  { key: 'gdpr', icon: ShieldCheckIcon },
  { key: 'eu', icon: EuFlag },
  { key: 'cookie_less', icon: CookieIcon },
  { key: 'open_source', icon: GithubIcon },
] as const;
...
export function Hero() {
  ...
  const heroPerks = perks.map((perk) => ({
    text: t(`hero_perks_${perk.key}`),
    icon: perk.icon,
  }));
      ...
      <div className="col w-full gap-8 sm:w-1/2 sm:pr-12">
        <div className="col gap-4">
          ...
          <h1 className="font-semibold text-4xl leading-[1.1] md:text-5xl">
            {t('hero_title_prefix')} <Competition />
          </h1>
